...
export default function Report() {
  ...
  return <div className="narrow"><span className="step">Question {step.progress.current} of {step.progress.total} · {Math.min(step.progress.percentage || 0, 100)}%</span><p className="question-breadcrumb">{step.breadcrumb?.join(' / ')}</p><h1>{question.title}</h1>{question.description && <p className="question-description">{question.description}</p>}<ErrorBox error={error}/><form onSubmit={answer}><QuestionRenderer question={question} value={value} setValue={setValue}/>{question.type !== 'terminal' && <div className="actions"><Button type="button" disabled={!step.navigation.canGoBack || busy} onClick={async () => { setValue(question.type === 'multi_select' ? [] : ''); setStep(await api.back(draft.draftId)) }}>Back</Button><Button primary disabled={busy || !hasQuestionAnswer(question, value)}>Continue</Button></div>}</form></div>
}
...
function AuthStep({ onDone, onBack }) {
  ...
  return <div className="narrow">{onBack && <button type="button" className="back-link" onClick={onBack}>← Back</button>}<span className="step">Authentication</span><h1>Before we continue, we need to verify you</h1><p>We need to verify you in order to submit your report.</p><ErrorBox error={error}/><form onSubmit={submit}><label>Mobile number<input value={phone} onChange={event => setPhone(event.target.value)} required minLength="10" maxLength="15" /></label>{sent && <label>OTP<input value={otp} onChange={event => setOtp(event.target.value)} required placeholder="1234" /></label>}<Button primary disabled={busy}>{busy ? 'Please wait…' : sent ? 'Verify OTP' : 'Send OTP'}</Button></form></div>
}
